...
function calendarDaySerial(key: string) { const [year, month, day] = key.split("-").map(Number); return Math.floor(Date.UTC(year, month - 1, day) / 86_400_000); }
function calendarDayDifference(startKey: string, endKey: string) { return calendarDaySerial(endKey) - calendarDaySerial(startKey); }
...
export function recordDailyRoutineOpening(routine: DailyRoutine, now = new Date()): DailyRoutineOpening { const currentDate = calendarDayKey(now); if (routine.lastOpenedDate === currentDate) return { routine, milestone: null }; const dayGap = routine.lastOpenedDate ? calendarDayDifference(routine.lastOpenedDate, currentDate) : null; const openedDayCount = dayGap === 1 ? Math.min(21, routine.openedDayCount + 1) : 1; const candidate = openedDayCount === 7 || openedDayCount === 14 || openedDayCount === 21 ? openedDayCount : null; const milestone = candidate && routine.lastCelebratedMilestone !== candidate ? candidate : null; return { routine: { ...routine, openedDayCount, lastOpenedDate: currentDate, lastCelebratedMilestone: milestone ?? routine.lastCelebratedMilestone }, milestone }; }
...
export function clearDailyHygieneReminder() { try { getStorage()?.removeItem(DAILY_HYGIENE_REMINDER_KEY); } catch { /* no storage available */ } }
export function isDailyHygieneReminderDue(reminder: DailyHygieneReminder, date = new Date()) { return reminder.lastPromptDate !== calendarDayKey(date); }
export function dismissDailyHygieneReminderForToday(reminder: DailyHygieneReminder, date = new Date()): DailyHygieneReminder { return { ...reminder, lastPromptDate: calendarDayKey(date) }; }
export function remainingDailyHygieneReminderDays(reminder: DailyHygieneReminder, now = new Date()) { const remaining = calendarDayDifference(calendarDayKey(now), calendarDayKey(new Date(reminder.endsAt))); return Math.max(1, remaining); }
export function getDailyHygienePlanProgress(reminder: DailyHygieneReminder, now = new Date()): DailyHygienePlanProgress { const startCalendarDay = calendarDayKey(new Date(reminder.startedAt)); const currentCalendarDay = calendarDayKey(now); const currentDay = Math.min(7, Math.max(1, calendarDayDifference(startCalendarDay, currentCalendarDay) + 1)); const completedDays = reminder.completedDayKeys.flatMap((key) => { const day = calendarDayDifference(startCalendarDay, key) + 1; return day >= 1 && day <= 7 ? [day] : []; }); const uniqueCompletedDays = Array.from(new Set(completedDays)).sort((left, right) => left - right); return { currentDay, completedDays: uniqueCompletedDays, completedCount: uniqueCompletedDays.length }; }
...
export function completeDailyHygieneForToday(reminder: DailyHygieneReminder, date = new Date()): DailyHygieneReminder { const key = calendarDayKey(date); return { ...reminder, lastPromptDate: key, completedDayKeys: reminder.completedDayKeys.includes(key) ? reminder.completedDayKeys : [...reminder.completedDayKeys, key] }; }
export function setDailyHygieneNoteForToday(reminder: DailyHygieneReminder, note: string, date = new Date()): DailyHygieneReminder { const key = calendarDayKey(date); const normalized = note.trim().slice(0, 1000); const completionNotes = { ...reminder.completionNotes }; if (normalized) completionNotes[key] = normalized; else delete completionNotes[key]; return { ...reminder, completionNotes }; }
...
export function restoreArchivedDailyHygienePlan(plan: ArchivedDailyHygienePlan): ArchivedDailyHygienePlan[] { const restored: ArchivedDailyHygienePlan = { ...plan, completedDayKeys: Array.from(new Set(plan.completedDayKeys)), completionNotes: { ...plan.completionNotes } }; const next = [restored, ...loadArchivedDailyHygienePlans().filter((existing) => existing.id !== restored.id)].sort((left, right) => Date.parse(right.archivedAt) - Date.parse(left.archivedAt)).slice(0, 30); writeJson(DAILY_HYGIENE_ARCHIVE_KEY, next); return next; }
